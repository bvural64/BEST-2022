...
#pragma config(Motor,  port7,           boxGrabberServo, tmotorServoStandard, openLoop)
...
task main()
{
	...
	int neutralGrabberPos = 0; // Variable for the front box grabber servo position
	motor[clawServo] = neutralClawPos; // Set claw to default before user control
	motor[boxGrabberServo] = neutralGrabberPos;
	...
	while (1 == 1)
	{
		...
		{
			motor[clawServo] = 127;
			neutralClawPos = motor[clawServo];
			wait1Msec(10);
		}
		...
		{
			motor[clawServo] = neutralClawPos;
		}
		...
		if (vexRT[Btn8L] == 1)
		{
			motor[boxGrabberServo] = 127;
			neutralGrabberPos = motor[boxGrabberServo];
			wait1Msec(10);
		}
